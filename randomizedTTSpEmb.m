function [G, time] = randomizedTTSpEmb(A, TTr, OV, q)
%%
tic;
...
A1 = bsxfun(@times, A, ind);
l1 = randsample(L, K, true);B = zeros(M(1), L);
for j = 1 : K
    B(:, l1(j)) = B(:, l1(j)) + A1(:, j);
end
for k=1 : q
...
for i = 2 : (N - 1)
    ...
    ind = randi(2, [1, K]) * 2 - 3;A1 = bsxfun(@times, A, ind);
    l1 = randsample(L, K, true);B = zeros(TTr(i-1) * M(i), L);
    for j = 1 : K
        B(:, l1(j)) = B(:, l1(j)) + A1(:, j);
    end
    for k=1 : q
